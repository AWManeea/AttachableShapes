...
{
    public class UShape : Shape
    {
        private Point offset;
        private Size size;
        private readonly Pen pen;
        private const ShapeType shapeType = ShapeType.UShape;
        ...
        public UShape(Point offset, Size size)
        {
            this.offset = offset;
            this.size = size;
            this.pen = new Pen(Color.Black, 7);
        }

        public override bool Accepts(ShapeType type)
        {
            return type == ShapeType.TShape;
        }

        public override GraphicsPath GetPath()
        {
            GraphicsPath gp = new GraphicsPath();
            gp.AddLine(new Point(X, Y), new Point(X + Width, Y));
            gp.AddLine(new Point(X + Width, Y+ Height / 3), new Point(X + Width, Y + Height / 3));
            gp.AddLine(new Point(X + 2 * Width / 3, Y + Height / 3), new Point(X + Width, Y + Height / 3));
            gp.AddLine(new Point(X + 2 * Width / 3, Y + Height / 3), new Point(X + 2 * Width / 3, Y + 2*Height/3));
            gp.AddLine(new Point(X + 2 * Width / 3, Y + 2*Height/3), new Point(X + Width / 3, Y + 2*Height/3));
            gp.AddLine(new Point(X + Width / 3, Y + 2*Height/3), new Point(X + Width / 3, Y + Height / 3));
            gp.AddLine(new Point(X + Width / 3, Y + Height / 3), new Point(X, Y + Height / 3));
            gp.AddLine(new Point(X, Y + Height / 3), new Point(X, Y));
            return gp;
        }
    }
}